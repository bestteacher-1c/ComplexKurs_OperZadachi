...
Процедура ОбработкаПроведения(Отказ, Режим)
	...
	Запрос.Текст = 
		...
		|					ИЗ
		|						ВТСписокНоменклатуры КАК ВТСписокНоменклатуры)) КАК СвободныеОстаткиОстатки
		|ГДЕ
			...
			#Область Запрос
				...
				Если Константы.МетодСписания.Получить() = Перечисления.МетодыСписанияСебестоимости.ЛИФО Тогда
					...
					Запрос.Текст = СтрЗаменить(Запрос.Текст,
					 "ВТСебестоимостьТоваровПоПартиям.Партия.МоментВремени ВОЗР",
					  "ВТСебестоимостьТоваровПоПартиям.Партия.МоментВремени УБЫВ");
