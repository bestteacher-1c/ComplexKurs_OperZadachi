...
Процедура Резерв(Команда)
	...
	Если Объект.Ссылка.Пустая() Тогда
		...
		//Описание асинхронного вызова
		КакаяПроцедураОтработаетПослеОтвета = Новый ОписаниеОповещения("РезервЗавершение", ЭтотОбъект);
		
		//Блокирующее окно
		ПоказатьВопрос(КакаяПроцедураОтработаетПослеОтвета,
		"Для резервирования надо записать документ. Записать?",
...
&НаКлиенте
Процедура РезервЗавершение(Ответ, ДополнительныеПараметры) Экспорт
...
Процедура РезервНаСервере()
	...
	//ДокументОбъект.ЗаполнитьНаборЗаписей_ТоварыВРезерве();
	
	//ДокументОбъект.Движения.ТоварыВРезерве.Записывать = Истина;
